...
```
원본:          space-to-depth 후 각 토큰의 채널 값
a b | c d      토큰(0,0): [a, b, e, f]   ← 픽셀 블록 [0:2, 0:2]
e f | g h      토큰(0,1): [c, d, g, h]   ← 픽셀 블록 [0:2, 2:4]
─────────      토큰(1,0): [i, j, m, n]   ← 픽셀 블록 [2:4, 0:2]
i j | k l      토큰(1,1): [k, l, o, p]   ← 픽셀 블록 [2:4, 2:4]
m n | o p
```

토큰 (i,j)에는 정확히 **픽셀 블록 [i×s:(i+1)×s, j×s:(j+1)×s]** 의 값만 포함.
Flux Fill은 block_size=8이므로 채널 64개 = VAE latent (i,j)가 인코딩하는 바로 그 영역.
